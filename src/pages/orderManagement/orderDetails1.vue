<template>
  <div class="contont" v-if="isDataDone">
    <div v-if="initData.status == 3">
        ...
        <el-button  v-if="isShow5 == false" type="primary" @click="exdeOrder(1)" >审核通过</el-button>
        <el-button  v-if="isShow5 == true" @click="exdeOrder(3)" type="danger"  >审核失败</el-button>
        <el-button  type="danger" @click="exdeOrder(2)" >取消订单</el-button>
        <el-button  type="primary setWidth" @click="showOtherWays" >失败,制作备选方案</el-button>
    </div>
    ...
    <div class="common">
      ...
      <div class="flex">
        <span>代   理</span>
        <span>
          <el-select v-model="initData.agentName"   filterable size="mini" placeholder="请选择">
            <el-option
    ...
    <div style="height:60vh">
      <div v-show="radio1=='1'" class="details">
            <div  class="addNewWays" v-if="isShow5">
              ...
              <!-- 审核失败 推荐制作方案 -->
              <div class="newFlag " v-for='(item,index) in initData.orderOptionsList' :key="index">
                  <div> 
                    <span> <i class="el-icon-star-on red"></i>起运港</span> 
                    <span>
                      <el-select v-model="item.pol" size="mini" :remote-method="polMethod"  clearable filterable remote reserve-keyword placeholder="请选择起运港" >
                        <el-option
                  ...
                  <div>
                    ...
                    <span>
                      <el-select v-model="item.pod" size="mini"  :remote-method="podMethod"  clearable filterable remote reserve-keyword placeholder="请选择目的港" >
                        <el-option
                  ...
                  <div>
                    <span><i class="el-icon-star-on red"></i>航&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;司</span>
                    <span>
                      <el-select v-model="item.airCompanyCode"  @change="getOrderAirName(item,index)" filterable size="mini" placeholder="请选择">
                        <el-option
                          v-for="item in airCompanyCodeList"
                          :key="item.airCompanyCode"
                          :label="item.airCompanyCode +'-'+ item.name "
                  ...
                  <div>
                    <span><i class="el-icon-star-on red"></i>日&nbsp;&nbsp;&nbsp;&nbsp;期</span>
                    <span>
                      <el-date-picker
                        size="mini"
                        style="width:193px"
                  ...
                  <div>
                    <span><i class="el-icon-star-on red"></i>单&nbsp;&nbsp;&nbsp;&nbsp;价</span>
                    <span>
                      <el-input  style="width:193px" v-model="item.bookingPrice" size="mini" placeholder="请输入内容"></el-input>  
                    </span>
                  </div>
                  <div>
                    <span><i class="el-icon-star-on red"></i>分泡比例</span>
                    <span>
                      <el-select    size="mini" v-model="item.bubblePoint"  placeholder="请选择">
                        <el-option
                  ...
                  <div>
                    <span><i class="el-icon-star-on red"></i>代 &nbsp;&nbsp;&nbsp;理</span>
                    <span>
                      <el-select v-model="item.agentId"   filterable size="mini" placeholder="请选择">
                        <el-option
                          v-for="item in agentIdList"
                          :key="item.id"
                          :label="item.agentName"
                  ...
                  <div >
                    <span> <i class="el-icon-star-on " style="color:white"></i>航班号</span>
                    <span><el-input  style="width:193px" v-model="item.flightNo" size="mini" placeholder="请输入航班号"></el-input></span>
                  </div>
                  <div >
                  ...
                  <el-button type="primary" @click="addOrderOne" v-if="initData.orderOptionsList.length - 1 == index && index != 2" size="mini">添加方案</el-button>
                  <el-button type="danger" @click="deleteOrderOne(index)" size="mini">删除方案</el-button>
              </div>
...
class orderSecondWays{
  constructor(pol,pod,airCompanyCode,agentId,bookingPrice,departureDate,flightNo,chooseRadio = '1'){
    this.pod = pod
    this.pol = pol
...
export default {
  ...
  methods:{
    ...
    calcFullLeg(e){
      return  e.chooseRadio == '1' ? `${e.pol},${e.pod}` : `${e.pol},中转,${e.pod}`
    },
    // 转换失败显示状态
    showOtherWays(){
    ...
    exdeOrder(e){
      ...
      if(this.isShow5){
          ...
          let tempArray1 = tempArray.filter(item=>{
            if(item.airCompanyCode == '' || item.agentId == '' || item.bookingPrice == '' || item.bookingPrice == '' || item.departureDate == ''){
              return item
            }
          })
    ...
    async initSysSetTing(){
      ...
      let res4 = await this.$http.post(this.$service.agentList,{pageSize: 50000,})
      let res5 = await  this.$http.get(this.$service.companySearchByPage+'?pageSize=50000')
