  ...
  <div class="content-wrapper">
    <div class="content">
      ...
      >
        ...
        >
          <div style="font-size: 18px; font-weight: 100; color: #2273ce">
            待客户确认备选方案
          </div>
          <div style="margin: 0 20px">
            <el-button style="width: auto" size="medium" type="primary"
              >取消订单</el-button
            >
          </div>
          <div style="font-size: 18px; font-weight: 100; color: #f00">
            {{ timeOut }}
          </div>
        </div>

        <!-- 客户信息 -->
        <div style="font-size: 18px; font-weight: 100; margin-bottom: 10px">
          客户信息
        </div>
        <div>
        ...
        <div>
          <el-form-item label="国内提货">
            <el-radio-group v-model="isPickUp">
              <el-radio
                :disabled="orderStatus.indexOf(status) > -1 ? false : true"
                v-for="(item, index) in isPickUpOpt"
                :key="index"
                :label="item.Value"
                >{{ item.Name }}</el-radio
              >
        ...
        <div>
          <el-form-item label="清关服务">
            <el-radio-group v-model="cclType">
              <el-radio
                :disabled="orderStatus.indexOf(status) > -1 ? false : true"
                v-for="(item, index) in cclTypeOpt"
                :key="index"
                :label="item.Value"
                >{{ item.Name }}</el-radio
              >
        ...
        <div v-if="cclType != '1'">
          <el-form-item label="送货联系人">
            <el-input
              :disabled="orderStatus.indexOf(status) > -1 ? false : true"
              v-model="deliveryContacts"
              placeholder="请输入送货联系人"
              maxlength="50"
              style="width: 596px"
            ></el-input>
          </el-form-item>
        ...
        <div>
          <el-form-item label="起运港">
            <el-select
              ...
              @change="initAirlineSearchByPage"
              :loading="loading"
              clearable
              filterable
              remote
        ...
        <div>
          <el-form-item label="航线路径">
            ...
            <div v-if="flightNoOpt.length == 0" style="color: #2273ce">
              选择起运港、目的港、航司、代理公司后才会有推荐航线
            </div>
            <el-radio-group
              v-else
              v-model="fullLeg"
            ...
            >
              <el-radio
                v-for="(optItem, optIndex) in flightNoOpt"
                :key="optIndex"
                :label="optItem.fullLeg"
                >{{ optItem.fullLeg }}</el-radio
              >
            </el-radio-group>
          </el-form-item>
        </div>
        ...
        <div>
          ...
          <el-form-item>
            <div style="display: flex">
              ...
              >
                <div @click="downLoadPDF(item)" style="text-align: center">
                  <img
                    src="../../assets/pdf.png"
                    alt=""
        ...
        >
          <div class="flight-template" style="width: auto; margin-left: 0">
            <div class="flight-template-ul-header">
              ...
              <div
                v-if="orderStatus.indexOf(status) > -1"
                class="flight-template-li"
                style="flex: 0 0 10%; text-align: center"
              >
                操作
              </div>
            </div>
            <div
              v-for="(childerItem, childerIndex) in orderCargoDetailList"
              :key="childerIndex"
            ...
            >
              ...
              <div class="flight-template-li" style="flex: 0 0 15%">
                <el-input
                  :disabled="orderStatus.indexOf(status) > -1 ? false : true"
                  v-model="childerItem.cbm"
                  @blur="cbmBlur(childerItem.cbm, childerIndex, '体积2')"
                  onkeyup="value=value.replace(/[^\d\.]/g, '')"
                  maxlength="9"
                  size="small"
                  style="width: 90%"
              ...
              >
                <a
                  :style="{
                    visibility:
                      orderCargoDetailList.length > 9 ? 'hidden' : 'visible',
                  }"
                  @click="addOrderCargoDetailList(childerIndex)"
                  style="font-size: 18px"
                  ><i class="el-icon-circle-plus-outline"></i
                ></a>
                <a
                  @click="delOrderCargoDetailList(childerIndex)"
                  style="font-size: 18px"
        ...
        <div>
          <el-form-item class="up-input" label="进仓图片">
            <div
              ...
              style="width: 800px; display: flex; flex-wrap: wrap"
            >
              <div
        ...
        <div
          v-for="(priceItem, priceIndex) in arOrderPriceList"
          :key="priceIndex"
          class="route-module"
          style="margin-left: 0; width: 90%; padding-bottom: 0"
        >
          <div class="flight-template" style="width: auto; margin-left: 0">
            <div class="flight-template-ul-header">
              <div
                class="flight-template-li"
                style="flex: 0 0 5%; text-align: center"
              >
                序号
              </div>
              <div
                class="flight-template-li"
                style="flex: 0 0 13%; text-align: center"
              >
                费用名称
              </div>
              <div
              ...
              <div
                class="flight-template-li"
                style="flex: 0 0 7%; text-align: center"
              >
                单价
              </div>
              <div
            ...
            <div
              v-for="(childerItem, childerIndex) in priceItem.list"
              :key="childerIndex"
              class="flight-template-ul-content"
            >
              ...
              <div class="flight-template-li" style="flex: 0 0 13%">
                <el-input
                  v-if="childerItem.expenseName == '空运费'"
                  value="空运费"
                  :disabled="true"
                  size="small"
                  style="width: 90%"
                ...
                <el-select
                  :disabled="orderStatus.indexOf(status) > -1 ? false : true"
                  v-else
                  v-model="childerItem.expenseName"
                  size="small"
                  clearable
                  placeholder="请选择"
                  style="width: 90%"
              ...
              <div class="flight-template-li" style="flex: 0 0 13%">
                <el-input
                  v-if="childerItem.expenseName == '空运费'"
                  :value="childerItem.expenseUnitName"
                  :disabled="true"
                  size="small"
                  style="width: 90%"
                ></el-input>
              ...
              <div class="flight-template-li" style="flex: 0 0 7%">
                ...
                <el-input
                  :disabled="orderStatus.indexOf(status) > -1 ? false : true"
                  v-else
                  v-model="childerItem.quantity"
                  @blur="
                    priceBlur(
                      childerItem.quantity,
                      childerIndex,
                      '应收',
                      '数量',
                      priceIndex
              ...
              <div class="flight-template-li" style="flex: 0 0 5%">
                <el-input
                  v-if="childerItem.expenseName == '空运费'"
                  :value="childerItem.exchangeRate"
                  :disabled="true"
                  size="small"
                  style="width: 90%"
                ></el-input>
                <el-input
                  :disabled="orderStatus.indexOf(status) > -1 ? false : true"
                  v-else
                  v-model="childerItem.exchangeRate"
                  @blur="
                    priceBlur(
                      childerItem.exchangeRate,
                      childerIndex,
                      '应收',
                      '汇率',
                      priceIndex
              ...
              >
                <a
                  :style="{
                    visibility:
                      arOrderPriceList.length > 9 ? 'hidden' : 'visible',
                  }"
                  @click="addArOrderPriceList(priceIndex, childerIndex)"
                  style="font-size: 18px"
                  ><i class="el-icon-circle-plus-outline"></i
                ></a>
                <a
                  @click="delArOrderPriceList(priceIndex, childerIndex)"
                  style="font-size: 18px"
                  ><i class="el-icon-delete"></i
                ></a>
              </div>
          ...
          <div>
            <el-form-item
              v-if="inboundPiece && status != '13' && priceItem.status == '0'"
              label=""
            >
              <el-button
                @click="duiZhangClick"
            ...
            <div
              v-if="inboundPiece && status != '13' && priceItem.status == '1'"
              style="
                color: #f00;
                font-size: 14px;
                padding-bottom: 20px;
            ...
            >
              ...
              <div style="margin-left: 20px; color: #2273ce; cursor: pointer">
                修改账单
              </div>
            </div>
            <div
              v-if="inboundPiece && status != '13' && priceItem.status == '2'"
              style="
                color: #f00;
                font-size: 14px;
                padding-bottom: 20px;
            ...
            <div
              v-if="inboundPiece && status != '13' && priceItem.status == '3'"
              style="
                color: #f00;
                font-size: 14px;
                padding-bottom: 20px;
            ...
            <div
              v-if="inboundPiece && status != '13' && priceItem.status == '4'"
              style="
                color: #f00;
                font-size: 14px;
                padding-bottom: 20px;
        ...
        >
          <div class="flight-template" style="width: auto; margin-left: 0">
            ...
            <div
              v-for="(childerItem, childerIndex) in apOrderPriceList"
              :key="childerIndex"
              class="flight-template-ul-content"
            >
              <div class="flight-template-li" style="flex: 0 0 5%">
              ...
              <div class="flight-template-li" style="flex: 0 0 7%">
                ...
                <el-input
                  ...
                  v-model="childerItem.price"
                  @blur="
                    priceBlur(childerItem.price, childerIndex, '应付', '单价')
                  "
                  onkeyup="value=value.replace(/[^\d\.]/g, '')"
                  size="small"
                  style="width: 90%"
              ...
              >
                <a
                  :style="{
                    visibility:
                      apOrderPriceList.length > 9 ? 'hidden' : 'visible',
                  }"
                  @click="addApOrderPriceList(childerIndex)"
                  style="font-size: 18px"
                  ><i class="el-icon-circle-plus-outline"></i
                ></a>
                <a
                  @click="delApOrderPriceList(childerIndex)"
                  style="font-size: 18px"
                  ><i class="el-icon-delete"></i
                ></a>
              </div>
          ...
          <div>
            <el-form-item
              v-if="inboundPiece && status != '13' && financeStatus == '0'"
              label=""
            >
              <el-button
                @click="jiaoDanClick"
            ...
            <el-form-item
              v-if="inboundPiece && status != '13' && financeStatus == '1'"
              label=""
            >
              <el-button
                @click="jiaoDanClick"
      ...
      >
        <div class="rest-style" style="padding-left: 20px">
          <el-form-item label=" " label-width="150px">
            <el-button
              @click="submitClick('保存')"
              style="height: 36px; line-height: 36px; padding: 0"
              type="primary"
              >保存</el-button
            >
          </el-form-item>
...
export default {
  ...
  watch: {
    inboundWeight() {
      if (this.inboundWeight && this.inboundCbm) {
        ...
        this.inboundCw =
          this.inboundCbm * 167 > this.inboundWeight
            ? Math.ceil(this.inboundCbm * 167)
            : Math.ceil(this.inboundWeight);
      } else {
        this.inboundVwr = "";
        this.inboundCw = "";
      }
  ...
  methods: {
      ...
      if (!reg.test(this.bookingPrice)) {
        ...
        for (var i = 0; i < this.apOrderPriceList.length; i++) {
          if (this.apOrderPriceList[i].expenseName == "空运费") {
            ...
            this.apOrderPriceList[i].totalOrgn = Math.ceil(
              this.arOrderPriceList[i].quantity * this.apOrderPriceList[i].price
            );
            this.apOrderPriceList[i].totalCny = Math.ceil(
              this.arOrderPriceList[i].quantity * this.apOrderPriceList[i].price
            );
          }
        }
        for (var q = 0; q < this.arOrderPriceList.length; q++) {
          if (this.arOrderPriceList[q].expenseName == "空运费") {
            ...
            this.arOrderPriceList[q].totalOrgn = Math.ceil(
              this.arOrderPriceList[q].quantity * this.arOrderPriceList[q].price
            );
            this.arOrderPriceList[q].totalCny = Math.ceil(
              this.arOrderPriceList[q].quantity * this.arOrderPriceList[q].price
            );
          }
        }
    ...
    bubblePointChang() {
      var shuliang = Math.ceil(
        (this.bubblePoint / 10) * this.bookingCw +
          (1 - this.bubblePoint / 10) * this.bookingWeight
      );
      for (var i = 0; i < this.apOrderPriceList.length; i++) {
        if (this.apOrderPriceList[i].expenseName == "空运费") {
          this.apOrderPriceList[i].quantity = shuliang;
          this.apOrderPriceList[i].totalOrgn = Math.ceil(
            shuliang * this.apOrderPriceList[i].price
          );
          this.apOrderPriceList[i].totalCny = Math.ceil(
            shuliang * this.apOrderPriceList[i].price
          );
      ...
      for (var q = 0; q < this.arOrderPriceList.length; q++) {
        if (this.arOrderPriceList[q].expenseName == "空运费") {
          ...
          this.arOrderPriceList[q].totalOrgn = Math.ceil(
            shuliang * this.arOrderPriceList[q].price
          );
          this.arOrderPriceList[q].totalCny = Math.ceil(
            shuliang * this.arOrderPriceList[q].price
          );
    ...
    submitClick(type) {
      ...
      if (this.showMake) {
        ...
        for (var q = 0; q < this.orderOptionsList.length; q++) {
          var json = {
            ...
            airCompanyCode:
              this.orderOptionsList[q].airCompanyCode.split("#")[0],
            airCompanyName:
              this.orderOptionsList[q].airCompanyCode.split("#")[1],
            ...
            dow:
              new Date(this.orderOptionsList[q].departureDate).getDay() == 0
                ? 7
                : new Date(this.orderOptionsList[q].departureDate).getDay(),
      ...
      for (var j = 0; j < this.orderCargoDetailList.length; j++) {
        if (
          this.orderCargoDetailList[j].piece &&
          this.orderCargoDetailList[j].cbm &&
          this.orderCargoDetailList[j].weight &&
          this.orderCargoDetailList[j].packing &&
          this.orderCargoDetailList[j].outerBox &&
          this.orderCargoDetailList[j].cargoSize
        ) {
      ...
      if (type == "应收") {
        ...
        for (var i = 0; i < this.arOrderPriceList.length; i++) {
          ...
          for (var q = 0; q < this.arOrderPriceList[i].list.length; q++) {
            this.arOrderPriceList[i].billAmountCny +=
              this.arOrderPriceList[i].list[q].totalCny;
          ...
          this.arOrderPriceList[i].totalArOrgn += value1
            ? "￥" + value1 + "+"
            : "";
          this.arOrderPriceList[i].totalArOrgn += value2
            ? "HK$" + value2 + "+"
            : "";
          this.arOrderPriceList[i].totalArOrgn += value3
            ? "$" + value3 + "+"
            : "";
          this.arOrderPriceList[i].totalArOrgn += value4
            ? "€" + value4 + "+"
            : "";
          this.arOrderPriceList[i].totalArOrgn += value5 ? "￡" + value5 : "";
          this.arOrderPriceList[i].totalArOrgn = this.arOrderPriceList[
            i
          ].totalArOrgn.substring(
            0,
            this.arOrderPriceList[i].totalArOrgn.length - 1
          );
        ...
        this.totalApOrgn = this.totalApOrgn.substring(
          0,
          this.totalApOrgn.length - 1
        );
      }
    },
    //账单单价、数量、汇率计算
      ...
      if (title == "单价") {
        if (!reg.test(num) && num != "") {
          ...
          if (
            type == "应收" &&
            this.arOrderPriceList[priceIndex].list[index].price &&
            this.arOrderPriceList[priceIndex].list[index].quantity
          ) {
            this.arOrderPriceList[priceIndex].list[index].totalOrgn = Math.ceil(
              this.arOrderPriceList[priceIndex].list[index].price *
                this.arOrderPriceList[priceIndex].list[index].quantity
            );
            if (
            ...
            type == "应付" &&
            this.apOrderPriceList[index].price &&
            this.apOrderPriceList[index].quantity
          ) {
            this.apOrderPriceList[index].totalOrgn = Math.ceil(
              this.apOrderPriceList[index].price *
                this.apOrderPriceList[index].quantity
            );
            if (this.apOrderPriceList[index].exchangeRate == "") {
              this.apOrderPriceList[index].totalCny = Math.ceil(
                this.apOrderPriceList[index].price *
                  this.apOrderPriceList[index].quantity *
                  1
              );
        ...
        if (!reg1.test(num) && num != "") {
          ...
          if (
            ...
            this.arOrderPriceList[priceIndex].list[index].totalCny = Math.ceil(
              this.arOrderPriceList[priceIndex].list[index].price *
                this.arOrderPriceList[priceIndex].list[index].quantity *
                1
            );
    ...
    initExpenseCode() {
      ...
      json = toData(json);
      this.$http
        .get(this.$service.expenseSearchExcludeAirFee + "?" + json)
        .then((data) => {
          if (data.code == 200) {
    ...
    initPrincipal() {
      ...
      json = toData(json);
      this.$http
        .get(this.$service.userSearchNoAuth + "?" + json)
        .then((data) => {
          if (data.code == 200) {
          ...
          if (data.code == 200) {
            ...
            if (data.apOrderPriceList) {
              for (var i = 0; i < data.apOrderPriceList.length; i++) {
                data.apOrderPriceList[i].currency =
                  data.apOrderPriceList[i].currency.toString();
              }
            }
            ...
            if (data.arOrderPriceList) {
              for (var r = 0; r < data.arOrderPriceList.length; r++) {
                for (var t = 0; t < data.arOrderPriceList[r].list.length; t++) {
                  data.arOrderPriceList[r].list[t].currency =
                    data.arOrderPriceList[r].list[t].currency.toString();
                }
              }
            }
            ...
            if (data.orderOptionsList != null) {
              if (data.orderOptionsList.length != 0) {
                ...
                for (var q = 0; q < this.orderOptionsList.length; q++) {
                  ...
                  this.orderOptionsList[q].bubblePoint =
                    data.orderOptionsList[q].bubblePoint.toString();
                  this.orderOptionsList[q].agentId =
                    data.orderOptionsList[q].agentId +
                    "#" +
                    data.orderOptionsList[q].agentName;
                }
              }
            }
            if (data.status == "5") {
              ...
              if (data.orderCargoDetailList.length != 0) {
                ...
                for (var z = 0; z < this.orderCargoDetailList.length; z++) {
                  if (this.orderCargoDetailList[z].packing != null) {
                    this.orderCargoDetailList[z].packing =
                      this.orderCargoDetailList[z].packing.toString();
                  }
                  if (this.orderCargoDetailList[z].outerBox != null) {
                    this.orderCargoDetailList[z].outerBox =
                      this.orderCargoDetailList[z].outerBox.toString();
                  }
                }
              }
            }
