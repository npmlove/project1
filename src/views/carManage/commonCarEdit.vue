...
<script>
  import { getShopTruckDetailsDs, getShopTruckUpdate, getComTableGet, getCIWXShopTruckUpdate } from '@/api/carList'
  export default {
    name: 'Form',
    data() {
      return {
    ...
    methods: {
      ...
      async toDel(row,index) {
        this.$baseConfirm('确认解绑【' + row.devicename + '】设备吗？', null, async () => {
          let params = {
            CallType: 'unbindaccessory',
            // stuid: row.stauid,
            UserID: sessionStorage.getItem('UserID'),
            keyid: row.ststaruid
          }
          const { Data, Status } = await getCIWXShopTruckUpdate(params)
