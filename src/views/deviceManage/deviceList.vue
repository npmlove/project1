    ...
    <div style="position: relative">
      <el-table ref="tableSort" v-loading="listLoading" :data="list" :element-loading-text="elementLoadingText" :height="height" border>
         <el-table-column  v-if="itemArr.indexOf('设备SN') > -1" label="设备SN" prop="devicesn" show-overflow-tooltip>
          ...
          <el-table-column v-if="itemArr.indexOf('操作') > -1" show-overflow-tooltip label="操作" width="300" >
            <template #default="{ row }">
              <el-button v-if="row.canRegistVideo == 1" :class="[row.videoRegister == 1 ? 'fontColor' : 'fontDisabledColor']" type="text" @click="registerVideoPlatform(row)">
              <!-- <el-button v-if="row.canRegistVideo == 1" :style="{ color: row.videoRegister == 1 ? '#41b584' : '#ccc' }" type="text" @click="registerVideoPlatform(row)"> -->
                {{ row.videoRegister == 1 ? '注册视频平台' : '视频平台已注册' }}
              </el-button>

               <el-button v-if="row.canRegistIOT == 1" :class="[row.iotRegister == 1 ? 'fontColor' : 'fontDisabledColor']" type="text" @click="registerInternetOfThings(row)">
              <!-- <el-button v-if="row.canRegistIOT == 1" :style="{ color: row.iotRegister == 1 ? '#41b584' : '#ccc' }" type="text" @click="registerInternetOfThings(row)"> -->
                {{ row.iotRegister == 1 ? '注册物联网平台' : '物联网平台已注册' }}
              </el-button>

               <el-button v-if="row.canBingdOrg == 1" :class="[row.corguid == '' ? 'fontColor' : 'fontDisabledColor']" type="text" @click="bindDevice(row)">
              <!-- <el-button v-if="row.canBingdOrg == 1" :style="{ color: row.corguid == '' ? '#41b584' : '#ccc' }" type="text" @click="bindDevice(row)"> -->
    ...
    <el-dialog title="分配设备" :visible.sync="dialogFormVisible" width="400px">
      <el-form ref="form" label-width="50px">
        <el-form-item label="组织" >
         ...
         <!-- <el-select placeholder="请选择组织" v-model="group" style="width: 270px" filterable>
            <el-option v-for="item in groupArr" :key="item.orguid" :label="item.orgname" :value="item.orguid"></el-option>
          </el-select> -->
        </el-form-item>
      </el-form>
...
<script>
  import { getDeviceList, getXMmdadd, getIOTDeviceRegister, getComOneLevel, accessoryOrgUpd } from '@/api/device'
  ...
  export default {
    ...
    methods: {
      ...
      //点击注册视频平台
      async registerVideoPlatform(row) {
        if (row.videoRegister != 1) { // 不等于1 说明已经注册过了
          return
        } else
        if(row.canRegistVideo != 1){ // 不等于1 没有权限
        ...
        const { Status, Message } = await getXMmdadd(json)
        if (Status == 0) {
      ...
      //点击注册物联网平台
      async registerInternetOfThings(row) {
        if (row.iotRegister != 1) { // 不等于1 说明已经注册过了
          return
        } else
        if(row.canRegistIOT != 1){  // 不等于1 没有权限
          this.$baseMessage('您没有操作权限，请联系管理员设置', 'error')
          return
        }
        this.listLoading = true
        ...
        const { Status, Message } = await getIOTDeviceRegister(json)
        if (Status == 0) {
          this.$baseMessage('注册成功', 'success')
        } else {
          this.$baseMessage(Message, 'error')
